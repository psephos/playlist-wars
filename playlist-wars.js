  ...
  common: {
    init: function() {
      // application-wide code
		...
		$('#scoreboard-close').click(function() {$('#scoreboard').removeClass('scoreboard-show');});
	...
	Engine: {
		...
		CountdownCounter: function() {
			...
			if (PlaylistWars.Engine.CountdownNumber == 0) {
				...
				// start ticking
				player.play('spotify:track:5jYHsgkOBMySJwcs7KKEok');
				PlaylistWars.Engine.Tick(200);
				return;
			}
			...
			var t = setTimeout('PlaylistWars.Engine.CountdownCounter()', 1000);		// game ticks
			var g = setTimeout('PlaylistWars.Engine.GameOver()', PlaylistWars.Util.Parameters.gameTime * 1000);	// game over in 30 seconds
		},
		PlayGame: function() {
			...
			PlaylistWars.Engine.GameOverFlag = false;
		...
		Tick: function(milliseconds) {
			...
			if (PlaylistWars.Engine.GameOverFlag) {	// stop if the game is over
				return;
			}
		...
		GameOverFlag: false,
		GameOver: function() {
			if (PlaylistWars.Engine.GameOverFlag) {
				return;
			}
			PlaylistWars.Engine.GameOverFlag = true;
			
			// who won?
			var winner = PlaylistWars.GameInfo.Scores.player1 > PlaylistWars.GameInfo.Scores.player2 ? PlaylistWars.GameInfo.Players[0] : PlaylistWars.GameInfo.Players[1];
			var winnerScore = PlaylistWars.GameInfo.Scores.player1 > PlaylistWars.GameInfo.Scores.player2 ? PlaylistWars.GameInfo.Scores.player1 : PlaylistWars.GameInfo.Scores.player2;
			var loserScore = PlaylistWars.GameInfo.Scores.player1 < PlaylistWars.GameInfo.Scores.player2 ? PlaylistWars.GameInfo.Scores.player1 : PlaylistWars.GameInfo.Scores.player2;
			var loser = PlaylistWars.GameInfo.Scores.player1 < PlaylistWars.GameInfo.Scores.player2 ? PlaylistWars.GameInfo.Players[0] : PlaylistWars.GameInfo.Players[1];
			var iWon = winner.spotifyAnonymousUserId == sp.core.getAnonymousUserId() ? true : false;
			
			// populate the scoreboard
			// and start playing the winning playlist
			if (iWon) {
				$('#scoreboard-header span').removeClass().html('You Won!').addClass('winner')
				$('#scoreboard-header img').attr('src', 'img/win.png');
				player.play('spotify:track:2GMJRFm2WT5Yj79So2SDmp');
				player.position = 34500;
			}
			else {
				$('#scoreboard-header span').removeClass().html('You Lose').addClass('loser')
				$('#scoreboard-header img').attr('src', 'img/lose.png');
				player.play('spotify:track:0sGNcx6lfc3zrbiIOxY4vb');
			}
			$('#scoreboard-player1-header span').html('Winner (' + winnerScore + ')');
			$('#scoreboard-player2-header span').html('Loser (' + loserScore + ')');
			var winnerPlaylist = new models.Playlist();
			var winnerUIPlaylist = new views.List(winnerPlaylist);
			$('#scoreboard-player1-playlist').html(winnerUIPlaylist.node);
			var loserPlaylist = new models.Playlist();
			var loserUIPlaylist = new views.List(loserPlaylist);
			$('#scoreboard-player2-playlist').html(loserUIPlaylist.node);
			$(".sp-list").addClass("sp-light");

			// load in the tracks
			for (var i = 0; i < winner.Tracks.length; i++) {
				winnerPlaylist.add(winner.Tracks[i].track);
			}
			for (var i = 0; i < loser.Tracks.length; i++) {
				loserPlaylist.add(loser.Tracks[i].track);
			}
			
			// show the scoreboard
			$('#countdown').hide();
			$('#gamespace').removeClass('gamespace-show');
			$('#scoreboard').addClass('scoreboard-show');
			
			// submit to ranking
			// create our object to send over
			var tracksToSend = new Array();
			for (var x = 0; x < PlaylistWars.GameInfo.Players[0].Tracks.length; x++) {
				var track = PlaylistWars.GameInfo.Players[0].Tracks[x];
				tracksToSend.push(track.trackUri);
			}
			var scoreToSend = {
				spotifyAnonymousId: sp.core.getAnonymousUserId(),
				submittedTimestamp: PlaylistWars.GameInfo.GameTimestamp,
				score: PlaylistWars.GameInfo.Scores.player1,
				tracks: tracksToSend
			}
			$.post(PlaylistWars.Util.Parameters.scoreServer + '/submit/score', scoreToSend);
			
			// if player 2 is not the computer, submit playlist and score for ranking
			if (PlaylistWars.GameInfo.Players[1].spotifyAnonymousUserId != 0) {
				// TODO
				// Submit to ranking
			}
		},
				...
				if (PlaylistWars.Decisions.InMeleeRange(source, nearestEnemy)) {
					if (PlaylistWars.Decisions.ShouldRetreat(source)) {
						...
						if (damage > 0) {
							...
							// scoring etc
							$('#countdown span').html('-' + damage);
							if (nearestEnemy.player == 1) {
								PlaylistWars.GameInfo.Scores.player1-= damage;
								PlaylistWars.GameInfo.Scores.player2+= 2 * damage;
								$('#countdown span').removeClass().addClass('player1-text');
							}
							else {
								PlaylistWars.GameInfo.Scores.player2-= damage;
								PlaylistWars.GameInfo.Scores.player1+= 2 * damage;
								$('#countdown span').removeClass().addClass('player2-text');
							}
							...
							if (nearestEnemy.health <= 0) {	// killed em
								...
								// change the display
								$('#countdown span').html('KILLED (-10)');
								if (nearestEnemy.player == 1) {
									$('#countdown span').removeClass().addClass('player1-text');
									PlaylistWars.GameInfo.Scores.player1-= 10;
									PlaylistWars.GameInfo.Scores.player2+= 20;
								}
								else {
									$('#countdown span').removeClass().addClass('player2-text');
									PlaylistWars.GameInfo.Scores.player2-= 10;
									PlaylistWars.GameInfo.Scores.player1+= 20;
								}
